...
export OPERATOR_NAME='ksd-container-security-operator'
export REGISTRY=${REGISTRY:-'quay.io'}
export NAMESPACE=${NAMESPACE:-'morana'}
...
docker build -t "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator:${TAG}" .
docker push "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator:${TAG}"
#digest "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator:${TAG}" OPERATOR_DIGEST

# need exporting so that yq can see them
export OPERATOR_DIGEST="${REGISTRY}/${NAMESPACE}/ksd-container-security-operator:${TAG}"
...
docker build -f ./bundle/Dockerfile -t "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator-bundle:${TAG}" ./bundle
docker push "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator-bundle:${TAG}"
digest "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator-bundle:${TAG}" BUNDLE_DIGEST

opm index add --build-tool docker --bundles "${BUNDLE_DIGEST}" --tag "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator-index:${TAG}"
docker push "${REGISTRY}/${NAMESPACE}/ksd-container-security-operator-index:${TAG}"
